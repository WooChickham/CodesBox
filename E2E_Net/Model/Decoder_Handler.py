...
class Decoder_Handler(Basement_Handler):
    ...
    def __init__(self, dataset_name, model_config, sess, is_training=True, is_testing_meas=False):  # zzh
        ...
        self.gen_test = Data_Generator_File(dataset_name, self.test_index, self.sense_mask, self.batch_size, self.nF,
                                            is_training=False, is_valid=False, is_testing=True,
                                            is_testing_meas=is_testing_meas)
    ...
    def train_test_valid_assignment(self):  # , is_training = True, reuse = False
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('Depth_Decoder', reuse=False):
                self.Decoder_train = Depth_Decoder(value_set, self.learning_rate, self.sess, self.model_config,
                                                   is_training=True)
        with tf.name_scope('Val'):
            with tf.variable_scope('Depth_Decoder', reuse=True):
                self.Decoder_valid = Depth_Decoder(value_set, self.learning_rate, self.sess, self.model_config,
                                                   is_training=False)
        with tf.name_scope('Test'):
            with tf.variable_scope('Depth_Decoder', reuse=True):
                self.Decoder_test = Depth_Decoder(value_set, self.learning_rate, self.sess, self.model_config,
                                                  is_training=False)
        ...
        while epoch_cnt <= self.epochs:
            ...
            for trained_batch in range(0, self.train_size):
                ...
                if trained_batch % 500 == 0 and trained_batch != 0:
                    ...
                    epoch_cnt, self.epochs, trained_batch, self.train_size, cur_lr, Train_loss, Train_psnr)
                    print(message)
            ...
            if valid_psnr >= max_val_psnr and valid_psnr >= 28:
                ...
                hdf5storage.write(matcontent, '.', self.log_dir + '/Validation_epoch%d.mat' % (epoch_cnt),
            ...
            if Vloss <= min_val_loss:
                ...
                self.logger.info(
                    'Val loss decrease from %.4f to %.4f, saving to %s' % (min_val_loss, Vloss, model_filename))
    ...
    def test(self):
        ...
        for tested_batch in range(self.test_size):
            ...
            start2 = time.time()
            feed_dict_test = {self.meas_sample: measure_test, self.sense_matrix: mask_train,
                              self.truth_seg: ground_test}
    ...
    def test_meas(self):
        ...
        for tested_batch in range(self.test_size):
            ...
            matcontent_v[u'pred'], matcontent_v[u'meas'] = np.squeeze(test_output['pred_orig']), np.squeeze(
                measure_test)
